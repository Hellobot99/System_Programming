...
void add_job(char **args, int pid);
...
void execute_command(char **args)
{
    ...
    if(strcmp(args[0], "fg") == 0)
    {
        ...
        {
            int z = atoi(args[1]) - 1;
            if(z < job_count && jobs[z].stopped){
                printf("Resuming job [%d] %s\n",z+1,jobs[z].command);
                int status;
                kill(recent_pid, SIGCONT);
                jobs[z].stopped = 0;
                waitpid(jobs[z].pid, &status, 0);
            }
            else
            {
                printf("Error\n");
        ...
        else
        {
            int z = -1;
            for(int i=0;i<job_count;i++){
                if(jobs[i].pid == recent_pid) z=i;
            }
            if(recent_pid != - 1 && z!=-1 && jobs[z].stopped){
                printf("Resuming job [%d] %s\n",z,jobs[z].command);
                int status;
                kill(recent_pid, SIGCONT);
                jobs[z].stopped = 0;
                waitpid(recent_pid, &status, 0);
                recent_pid = -1;
            }
            else
            {
